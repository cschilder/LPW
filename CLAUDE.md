...
**LPW — Lichtgewicht Internet Protocollen** is a static HTML5 website documenting 12 lightweight internet protocols (MQTT, CoAP, WebSocket, AMQP, HTTP/2, HTTP/3, XMPP, STOMP, DDS, LwM2M, MQTT-SN, LoRaWAN). The UI is in Dutch; technical terms and code examples are in English.
...
js/protocols.js     Single PROTOCOLS object — all protocol data and Markdown content
...
Add one entry to the `PROTOCOLS` object in `js/protocols.js` following the exact same shape as existing entries. The five `content` keys must be: `beschrijving`, `werking`, `toepassingen`, `opties`, `voorbeeld`. The site renders everything else automatically — no changes to `index.html` or `app.js` are needed.
